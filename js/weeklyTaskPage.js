...
async function refreshTasks() {
  const tasks = await getAllItems("weeklyTasks");
  tasks.sort((a,b)=> a.endTime.localeCompare(b.endTime));
  tasks.sort((a,b)=> a.startTime.localeCompare(b.startTime));
  tasks.sort((a,b)=> a.weekday.localeCompare(b.weekday));
...
function createTaskRow(task) {
  ...
  const index = Number(task.weekday);
  const weekdayText = weekdays[index];
  taskRow.querySelector(".cell-weekday").textContent = weekdayText || "";
  taskRow.querySelector(".cell-start").textContent = task.startTime || "";
  taskRow.querySelector(".cell-end").textContent = task.endTime || "";
  taskRow.querySelector(".cell-deadline").textContent = task.deadline + "日後" || "";
...
taskTableBody.addEventListener("click", (e) => {
  ...
  if (e.target.matches('[data-action="edit"]')) {
    ...
    openUpdateModal(row.dataset.id, "weekly")
  }
  // 削除ボタンが押された場合
  if (e.target.matches('[data-action="delete"]')) {
    const row = e.target.closest("tr");
    deleteItem(row.dataset.id, "weeklyTasks");
    refreshTasks();
  }
});