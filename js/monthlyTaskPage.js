...
function createTaskRow(task) {
  ...
  taskRow.querySelector(".cell-startday").textContent = task.startDay + "日" || "";
  taskRow.querySelector(".cell-start").textContent = task.startTime || "";
  taskRow.querySelector(".cell-end").textContent = task.endTime || "";
  taskRow.querySelector(".cell-deadline").textContent = task.deadline + "日" || "";
...
taskTableBody.addEventListener("click", (e) => {
  ...
  if (e.target.matches('[data-action="edit"]')) {
    ...
    openUpdateModal(row.dataset.id, "monthly")
  }
  // 削除ボタンが押された場合
  if (e.target.matches('[data-action="delete"]')) {
    const row = e.target.closest("tr");
    deleteItem(row.dataset.id, "monthlyTasks");
    refreshTasks();
  }
});